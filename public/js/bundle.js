...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiY2hpbm5hd2F0cG9rIiwiYSI6ImNrYmFlYm50NzA0bm4yc24zY2drb210dWUifQ.2lRCJhWF-eQuHrx6jXAeLg';
  var map = new mapboxgl.Map({
    container: 'map',
    style: 'mapbox://styles/chinnawatpok/ckbaepzqd0gk91ilgyvmtsv7l',
    scrollZoom: false // Lat Long
  ...
  locations.forEach(function (loc) {
    // create marker
    var el = document.createElement('div'); // marker have in CSS
    ...
    new mapboxgl.Popup({
      offset: 30
    }).setLngLat(loc.coordinates).setHTML("<p>Day ".concat(loc.day, " : ").concat(loc.description, "<p>")).addTo(map); // Entend map bounds to include current location
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            //console.log('555');
            console.log("".concat(email, " : ").concat(password));
            _context.prev = 1;
            _context.next = 4;
    ...
    return regeneratorRuntime.wrap(function _callee$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            _context.prev = 0;
            currentUrl = type === 'password' ? 'http://127.0.0.1:3000/api/v1/users/updateMyPassword' : 'http://127.0.0.1:3000/api/v1/users/updateMe';
            _context.next = 4;
            return (0, _axios.default)({
              method: 'PATCH',
              url: currentUrl,
...
var stripe = Stripe('pk_test_51GuCzQBK4Yygkx9LTI1h2vfloCu1hRxVWGc3t1BTDn0qYSPlvArRQ1spfCTPNiczUGBFmgXDgOazJOLvl7Txw4Jx00DV3jf6ux');
      ...
      while (1) {
        switch (_context.prev = _context.next) {
          ...
          case 3:
            ...
            console.log(session); // 2) Create checkput from + change credit card
...
// DOM ELEMENT
var mapBox = document.getElementById('map'); //const loginForm = document.querySelector('.form');
...
var bookBtn = document.getElementById('book-tour'); //VALUE
//  DELEGATION
...
if (updateDataForm) {
  updateDataForm.addEventListener('submit', function (e) {
    e.preventDefault();
    var form = new FormData(); //  เพิ่มไปต่อหลัง

    form.append('name', document.getElementById('name').value);
    form.append('email', document.getElementById('email').value); // const email = document.getElementById('email').value;
    // const email = document.getElementById('name').value;

    form.append('photo', document.getElementById('photo').files[0]);
    console.log(form); // updateSettings({ name, email }, 'data');
        ...
        while (1) {
          switch (_context.prev = _context.next) {
            case 0:
              ...
              password = document.getElementById('password').value; // เราจะล้าง field เราเลยต้องรอให้ ด้านล่างมันทำเสร็จ เพราะด้านล่างเป็น async function เราเลยรอให้เสร็จก่อนแล้วค่อย ลบค่าใน field
...
if (bookBtn) {
  ...
  bookBtn.addEventListener('click', function (e) {
    e.target.textContent = 'Process...'; // tourId มาจาก (data-tour-id).... มันจะแปลงเป็น camelCase ให้เอง
